...
Genera conjunts d                            ici per practicar amb el sistema
de fitxers, re                                   directori on es guardaran els
fitxers (per d              J

Opcions gen                               o
  -c NUM              y
                                         -D.
                                      xers existents sense demanar confirmació.
  -q         f                     ós (només errors).
  -v                         rbós (mostra cada fitxer creat).
  -V, --v                stra la versió i surt.
...
Tipus de fitxers (mútuament excloents):
  ...
  -B N      
               ...
               R
                                   ...
                                   rs datats (NOM_YY-MM.EXT)
                      
                           sió de sortida (per defecte: txt)
                        ata d’inici (per defecte: 00/01)
                       Data de fi (per defecte: mes actual)

  -U NOM               Genera fitxers d’usuaris (NOM_#.EXT)
                        ada línia té el format "<id><sep><nom><sep><cognoms><sep><naixement>".
                            sió de sortida (per defecte: txt)
            a                       cel·les (per defecte: ":")
                     u                       per defecte: 128)

            y                               tes sense extensió.
                                        ibles:
                                 _musicals, plantes, vaixella,
